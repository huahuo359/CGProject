...
const glm::vec3& MY_Camera::getPosition() const {
    return m_position;
}
void MY_Camera::setPosition(const glm::vec3& pos) {
    m_position = pos;
}

void MY_Camera::look(const glm::vec3& at) {
    glm::vec3 up(0.0f, 1.0f, 0.0f);
    m_view_matrix = glm::lookAt(m_position, at, up);
}

void MY_Camera::look(const glm::vec3& from, const glm::vec3& at) {
    glm::vec3 up(0.0f, 1.0f, 0.0f);
    m_view_matrix = glm::lookAt(from, at, up);
}

glm::mat4 MY_Camera::getInverted(float pivotPoint) {
