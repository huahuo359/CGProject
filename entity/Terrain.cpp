...
const float Terrain::TERRAIN_SIZE = 301.43f;
const float Terrain::TERRAIN_MAX_HEIGHT = 10.0f;

Terrain::Terrain(MY_Model* model, const vector<GLuint>& textures, Image heightMap)
...
Terrain* Terrain::loadTerrain(const std::vector<std::string>& images, const std::string& heightMapFile) {
    ...
    MY_Model* model = Terrain::generateTerrainModel(heightMap);
...
MY_Model* Terrain::generateTerrainModel(const Image& heightMap) {
